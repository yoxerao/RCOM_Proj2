...
int ftpPasv(FTP* ftp) {
	...
	if ((ftp->data_sckt_fd = connectSocket(pasv, portResult)) < 0) {
		printf(
...
int ftpDownload(FTP* ftp, const char* filename) {
	...
	char buf[1024];
	while ((bytes = read(ftp->data_sckt_fd, buf, sizeof(buf)))) {
		if (bytes < 0) {
...
int ftpRead(FTP* ftp, char* str, size_t size) {
	FILE* fp = fdopen(ftp->ctrl_sckt_fd, "r");
